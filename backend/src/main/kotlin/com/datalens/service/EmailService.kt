...
class EmailService(
    ...
    @Value("\${datalens.email.enabled:false}")
    private var emailEnabled: Boolean = false

    @Value("\${datalens.email.smtp-host:}")
    private var smtpHost: String = ""

    @Value("\${datalens.email.smtp-port:587}")
    private var smtpPort: Int = 587

    @Value("\${datalens.email.username:}")
    private var smtpUsername: String = ""

    @Value("\${datalens.email.password:}")
    private var smtpPassword: String = ""

    @Value("\${datalens.email.from:noreply@datalens.local}")
    private var fromAddress: String = "noreply@datalens.local"

    @Value("\${datalens.email.use-tls:true}")
    ...
    fun sendReportEmail(request: EmailDeliveryRequest, username: String): EmailDeliveryResponse {
        ...
        val subject = request.subject ?: "DataLens Report: ${report.name}"
        ...
        return try {
            ...
            for (recipient in request.recipients) {
                val message = MimeMessage(session).apply {
                    setFrom(InternetAddress(fromAddress, "DataLens"))
