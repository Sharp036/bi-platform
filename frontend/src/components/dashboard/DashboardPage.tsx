...
export default function DashboardPage() {
  ...
  const stats = [
    { icon: FileBarChart, label: 'Reports', value: reports.length, color: 'text-brand-600 bg-brand-50 dark:bg-brand-900/30' },
    { icon: Database, label: 'Data Sources', value: datasources.length, color: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30' },
    { icon: Code2, label: 'Saved Queries', value: queryCount, color: 'text-purple-600 bg-purple-50 dark:bg-purple-900/30' },
    { icon: TrendingUp, label: 'Published', value: reports.filter(r => r.status === 'PUBLISHED').length, color: 'text-amber-600 bg-amber-50 dark:bg-amber-900/30' },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between px-4 py-3 border-b border-surface-200 dark:border-dark-surface-100">
            <h2 className="font-semibold text-slate-700 dark:text-slate-200">Recent Reports</h2>
          ...
          <div className="divide-y divide-surface-200 dark:divide-dark-surface-100">
            {reports.length === 0 ? (
              <p className="px-4 py-6 text-sm text-slate-400 text-center">No reports yet</p>
            ) : reports.map(r => (
              <Link key={r.id} to={`/reports/${r.id}`}
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{r.widgets?.length || 0} widgets · {new Date(r.updatedAt).toLocaleDateString()}</p>
                </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between px-4 py-3 border-b border-surface-200 dark:border-dark-surface-100">
            <h2 className="font-semibold text-slate-700 dark:text-slate-200">Data Sources</h2>
          ...
          <div className="divide-y divide-surface-200 dark:divide-dark-surface-100">
            {datasources.length === 0 ? (
              <p className="px-4 py-6 text-sm text-slate-400 text-center">No data sources</p>
            ) : datasources.map(ds => (
